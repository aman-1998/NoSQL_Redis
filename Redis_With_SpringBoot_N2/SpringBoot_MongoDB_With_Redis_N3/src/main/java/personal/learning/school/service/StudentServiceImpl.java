...
public class StudentServiceImpl implements StudentService {
	...
	public Student getStudentDetails(String id) {
		...
		if(isPresentInCache) {
			...
			Student student = studentRepositoryMongo.getStudentById(id);
			if(student != null) {
	...
	@Override
	public Student getStudentDetailsVersion2(String id) {
		
		Student student = studentRepositoryRedis.getStudentById(id);
		
		if(student == null) {
			student = studentRepositoryMongo.getStudentById(id);
			if(student != null) {
				studentRepositoryRedis.save(student);
			}
		}
